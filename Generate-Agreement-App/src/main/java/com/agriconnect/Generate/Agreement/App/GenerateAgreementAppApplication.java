...
public class GenerateAgreementAppApplication {
	...
	private static void loadEnvironmentVariables() {
		try {
			...
			// Check if we're running from a subdirectory (like Generate-Agreement-App)
			if (!Files.exists(Paths.get(projectRoot, ".env"))) {
				// Look for .env file in the current module directory
				String moduleName = "Generate-Agreement-App";
